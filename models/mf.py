...
class MF(object):
    ...
    def _build_graph(self):
        ...
        with tf.compat.v1.variable_scope('mf'):# 声明变量作用域
            ...
            self.user_idx = tf.compat.v1.placeholder(tf.compat.v1.int32, [None])
            self.item_idx = tf.compat.v1.placeholder(tf.compat.v1.int32, [None])
            ...
            self.zc_user_embeddings = tf.compat.v1.concat(zc_user_embeddings, 1)
            zc_item_embeddings = tf.compat.v1.Variable(tf.compat.v1.random_normal([self._num_items, self._embed_dim],
                                                             stddev=1 / (self._embed_dim ** 0.5), dtype=tf.float32))
            

            self.zc_item_embeddings = tf.compat.v1.concat(zc_item_embeddings, 1)
