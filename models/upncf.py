...
class Objective:
    ...
    def __call__(self, trial: Trial) -> float:
        ...
        model = UPNCF(self.num_users, self.num_items, np.int(rank), np.int(batch_size), lamb=lam, alpha=alpha,
...
class UPNCF(object):
    ...
    def train_model(self, matrix_train, matrix_valid, matrix_test, epoch=3, metric='AUC', topK=50,
...
def upncf(matrix_train, matrix_valid, matrix_test, embeded_matrix=np.empty(0), iteration=1000, lam=0.01, alpha=0.01,
           rank=200, gamma=0.01, batch_size=500, learning_rate=1e-3, optimizer="Adam", seed=0, gpu_on=False,
           metric='AUC', topK=50, is_topK=False, searcher='optuna', n_trials=3, **unused):
    progress = WorkSplitter()

    progress.section("UPNCF: Set the random seed")
    np.random.seed(seed)
    tf.set_random_seed(seed)

    progress.section("UPNCF: Training")
    ...
    if searcher == 'grid':
        model = UPNCF(m, n, rank, batch_size, lamb=lam, alpha=alpha, gamma=gamma, learning_rate=learning_rate,
