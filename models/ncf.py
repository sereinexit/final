...
class Objective:
    ...
    def __call__(self, trial: Trial) -> float:
        ...
        model = NCF(self.num_users, self.num_items, np.int(rank), np.int(batch_size), lamb=lam, alpha=alpha,
...
class NCF(object):
    ...
    def _build_graph(self):
        ...
        with tf.variable_scope('ncf'):
            ...
            # Variable to learn
            zc_user_embeddings = tf.Variable(tf.random_normal([self._num_users, self._embed_dim],
            ...
            self.zc_user_embeddings = tf.concat([zc_user_embeddings, user_zero_vector], 1)

            zc_item_embeddings = tf.Variable(tf.random_normal([self._num_items, self._embed_dim],
                                                             stddev=1 / (self._embed_dim ** 0.5), dtype=tf.float32))
            item_zero_vector = tf.get_variable(
                'item_zero_vector', [self._num_items, self._embed_dim],
                initializer=tf.constant_initializer(0.0, dtype=tf.float32), trainable=False)

            self.zc_item_embeddings = tf.concat([zc_item_embeddings, item_zero_vector], 1)
            ...
            with tf.variable_scope("ncf_loss"):
                ...
                self.sigmoid_zc_x_ij = tf.nn.sigmoid(zc_x_ij)
    ...
    def train_model(self, matrix_train, matrix_valid, matrix_test, epoch=100, metric='AUC', topK=50,
        ...
        for i in tqdm(range(epoch)):
            ...
            valid_rating_prediction = self.sess.run(self.sigmoid_zc_x_ij,
            ...
            for step in range(len(all_batches)):
                ...
                vector_predict = self.sess.run([self.sigmoid_zc_x_ij], feed_dict=feed_dict)
            ...
            test_rating_prediction = self.sess.run(self.sigmoid_zc_x_ij,
...
def ncf(matrix_train, matrix_valid, matrix_test, embeded_matrix=np.empty(0), iteration=1000, lam=0.01, alpha=0.01,
    ...
    progress.section("NCF: Set the random seed")
    np.random.seed(seed)
    tf.set_random_seed(seed)

    progress.section("NCF: Training")
    ...
    if searcher == 'grid':
        model = NCF(m, n, rank, batch_size, lamb=lam, alpha=alpha, gamma=gamma, learning_rate=learning_rate,
